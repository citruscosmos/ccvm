...
`setup.sh` runs 13 sequential steps, each idempotent (skips if already done):
...
10. **Claude Code authentication** — 3-way choice: API key (pay-as-you-go), `claude login` OAuth (Pro/Max plans), or skip. `--skip auth` flag.
11. **tmux** — opinionated config (prefix C-a, mouse support, 256-color)
12. **claude-model** — installs session-only DeepSeek v4 launcher to `~/.local/bin/claude-model`
13. **Verification** — checks every tool was installed; shows next steps
